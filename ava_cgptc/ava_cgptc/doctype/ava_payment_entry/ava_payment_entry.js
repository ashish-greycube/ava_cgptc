...
frappe.ui.form.on('Ava Payment Entry', {
	...
	show_general_ledger: function(frm) {
		if(frm.doc.docstatus==1) {
			frm.add_custom_button(__('Ledger'), function() {
				...
				frappe.set_route("query-report", "Ava General Ledger");
			}, "fa fa-table");
		}
	},
	...
	party: function(frm) {
		...
		if(frm.doc.payment_type && frm.doc.party_type && frm.doc.party) {
			...
			return frappe.call({
				method: "ava_cgptc.ava_cgptc.doctype.ava_payment_entry.ava_payment_entry.get_party_details",
				args: {
					company: frm.doc.company,
	...
	set_account_currency_and_balance: function(frm, account, currency_field,
		...
		if (frm.doc.posting_date && account) {
			frappe.call({
				method: "ava_cgptc.ava_cgptc.doctype.ava_payment_entry.ava_payment_entry.get_account_details",
				args: {
					"account": account,
					"date": frm.doc.posting_date,
					"cost_center": frm.doc.cost_center
	...
	get_outstanding_documents: function(frm, filters) {
		...
		return  frappe.call({
			method: 'ava_cgptc.ava_cgptc.doctype.ava_payment_entry.ava_payment_entry.get_outstanding_reference_documents',
			args: {
				args:args
			},
			callback: function(r, rt) {
				if(r.message) {
					...
					if(
						...
						(frm.doc.payment_type=="Receive" && frm.doc.party_type=="Customer Group") ||
	...
	allocate_party_amount_against_ref_docs: function(frm, paid_amount) {
		...
		} else if (in_list(["Customer", "Supplier","Customer Group"], frm.doc.party_type)) {
	...
	set_deductions_entry: function(frm, account) {
		if(frm.doc.difference_amount) {
			frappe.call({
				method: "ava_cgptc.ava_cgptc.doctype.ava_payment_entry.ava_payment_entry.get_company_defaults",
				args: {
					company: frm.doc.company
				},
				callback: function(r, rt) {
	...
	bank_account: function(frm) {
		...
		if (frm.doc.bank_account && in_list(['Pay', 'Receive'], frm.doc.payment_type)) {
			frappe.call({
				method: "ava_cgptc.ava_cgptc.doctype.bank_account.bank_account.get_bank_account_details",
				args: {
...
frappe.ui.form.on('Ava Payment Entry Reference', {
	...
	reference_name: function(frm, cdt, cdn) {
		...
		if (row.reference_name && row.reference_doctype) {
			return frappe.call({
				method: "ava_cgptc.ava_cgptc.doctype.ava_payment_entry.ava_payment_entry.get_reference_details",
				args: {
...
frappe.ui.form.on('Ava Payment Entry', {
	cost_center: function(frm){
		if (frm.doc.posting_date && (frm.doc.paid_from||frm.doc.paid_to)) {
			return frappe.call({
				method: "ava_cgptc.ava_cgptc.doctype.ava_payment_entry.ava_payment_entry.get_party_and_account_balance",
				args: {
					company: frm.doc.company,
					date: frm.doc.posting_date,
