...
class AvaPaymentEntry(AccountsController):
	def __init__(self, *args, **kwargs):
		super(AvaPaymentEntry, self).__init__(*args, **kwargs)
		if not self.is_new():
	...
	def validate_reference_documents(self):
		...
		elif self.party_type in ["Customer","Customer Group"]:
			...
			if d.reference_doctype not in valid_reference_doctypes:
				...
				if not frappe.db.exists(d.reference_doctype, d.reference_name):
					...
					if d.reference_doctype in ("Sales Invoice", "Purchase Invoice", "Expense Claim", "Fees"):
						if self.party_type  in ["Customer","Customer Group"]:
	...
	def clear_unallocated_reference_document_rows(self):
		...
		frappe.db.sql("""delete from `tabAva Payment Entry Reference`
	...
	def set_remarks(self):
		...
		if self.payment_type in ["Receive", "Pay"]:
			for d in self.get("references"):
				if d.allocated_amount:
					remarks.append(_("Amount {0} {1} against {2} {3} for customer {4}").format(self.party_account_currency,
						d.allocated_amount, d.reference_doctype, d.reference_name,d.party))
	...
	def add_party_gl_entries_old(self, gl_entries,d_party,d_reference_doctype,d_reference_name,d_allocated_amount,d_exchange_rate):
		if self.party_account:
			...
			gle.update({
				"party":d_party,
				"party_type": "Customer" if self.party_type =='Customer Group' else self.party_type,
				"against_voucher_type": d_reference_doctype,
				"against_voucher": d_reference_name
			})

			allocated_amount_in_company_currency = flt(flt(d_allocated_amount) * flt(d_exchange_rate),
				self.precision("paid_amount"))
	...
	def add_bank_gl_entries_old(self, gl_entries,d_party,d_reference_doctype,d_reference_name,d_allocated_amount,d_exchange_rate):
		...
		allocated_amount_in_company_currency = flt(flt(d_allocated_amount) * flt(d_exchange_rate), self.precision("paid_amount"))			
		if self.payment_type in ("Pay", "Internal Transfer"):		
			gle.update({
				"credit_in_account_currency": allocated_amount_in_company_currency,
				"credit": d_allocated_amount
			})
		if self.payment_type in ("Receive", "Internal Transfer"):
			gle.update({
				"debit_in_account_currency": allocated_amount_in_company_currency,
				"debit": d_allocated_amount
			})
		gl_entries.append(gle)
...
def get_outstanding_reference_documents(args):
	...
	if args.get("party_type"):
		if args["party_type"]=='Customer Group' and args.get("party"):
			lft, rgt = frappe.db.get_value("Customer Group", args.get("party"), ['lft', 'rgt'])
	...
	if args.get("party_type") not in ["Student", "Employee"] and not args.get("voucher_no"):
		for party in args.get("party"):
			negative_outstanding_invoice=get_negative_outstanding_invoices(args.get("party_type"),party[0],
			args.get("party_account"), args.get("company"), party_account_currency, company_currency)
			if len(negative_outstanding_invoice)>0:
				for d in negative_outstanding_invoice:
					d['party']=party[0]
				negative_outstanding_invoices.append(negative_outstanding_invoice[0])
	...
	if (args.get("party_type") != "Student"):
		for party in args.get("party"):
			order_to_be_billed=get_orders_to_be_billed(args.get("posting_date"),args.get("party_type"),
			party[0], args.get("company"), party_account_currency, company_currency, filters=args)
			if len(order_to_be_billed)>0:
...
def get_orders_to_be_billed(posting_date, party_type, party,
	...
	if party_type in ["Customer","Customer Group" ]:
...
def get_negative_outstanding_invoices(party_type, party, party_account,
	...
	voucher_type = "Sales Invoice" if party_type in ["Customer","Customer Group" ] else "Purchase Invoice"
	supplier_condition = ""
