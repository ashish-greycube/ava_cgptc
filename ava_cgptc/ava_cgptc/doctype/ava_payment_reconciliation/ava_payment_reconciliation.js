...
frappe.ui.form.on("Ava Payment Reconciliation Payment", {
	invoice_number: function(frm, cdt, cdn) {
		...
		if(row.invoice_number) {
			...
			var party = frm.doc.invoices.filter(function(d) {
				return d.invoice_type === invoice_type && d.invoice_number === invoice_number;
			})[0].party;	
...
erpnext.accounts.PaymentReconciliationController = frappe.ui.form.Controller.extend({
		...
		if (show_dialog && show_dialog.length) {
			...
			const dialog = new frappe.ui.Dialog({
				...
				primary_action: function() {
					...
					args.forEach(d => {
						frappe.model.set_value("Ava Payment Reconciliation Payment", d.docname,
	...
	set_invoice_options: function() {
		...
		if (invoices) {
			frappe.meta.get_docfield("Ava Payment Reconciliation Payment", "invoice_number",
